...
Inference:
    $ python path/to/CIN.py --weights yolov5s.pt
                                         yolov5s.torchscript
                                         yolov5s.onnx
...
TensorFlow.js:
    $ cd .. && git clone https://github.com/zldrobit/tfjs-yolov5-example.git && cd tfjs-opt_version-example
    $ npm install
    $ ln -s ../../opt_version/yolov5s_web_model public/yolov5s_web_model
    $ npm start
"""
...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        LOGGER.info(f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        LOGGER.info(f"{prefix} run --dynamic ONNX model inference with: 'python CIN.py --weights {f}'")
    except Exception as e:
        LOGGER.info(f'{prefix} export failure: {e}')
